...
# Динамическое расписание для learning_rate
def lr_schedule(progress_remaining: float) -> float:
    return 3e-3 * progress_remaining  # Уменьшается по мере обучения

# Динамическое расписание для clip_range
def clip_range_schedule(progress_remaining: float) -> float:
    return 0.3 * progress_remaining + 0.1  # Начинаем с 0.4 и уменьшаем до 0.1
