...
def predict_proba_trpo(model, state, episode_start):
    """
    Предсказание вероятностей действий для TRPO
    Args:
        model: Модель TRPO
        state: Текущее наблюдение
        episode_start: Начало эпизода
    Returns:
        numpy массив вероятностей для станций
    """
    episode_starts = th.tensor(episode_start, dtype=th.float32, device=model.policy.device)
    obs = obs_as_tensor(state, model.policy.device)

    # Получаем распределение действий из политики TRPO
    with th.no_grad():
        dist = model.policy.get_distribution(obs)
    
    # Извлекаем вероятности из второго подмассива (индекс 1)
    station_probs = dist.distribution[1].probs  # Берем второй элемент списка (Categorical с logits [1, 3])
    station_probs = station_probs.squeeze()  # Удаляем лишние размерности (из [1, 3] в [3])
    
    return station_probs.detach().cpu().numpy()  # Возвращаем одномерный массив [3]

def add_bar_chart_to_image_trpo(img, probs):
    """
    Добавление столбчатой диаграммы вероятностей к изображению для TRPO
    Args:
        img: Исходное изображение (PIL Image)
        probs: Вероятности действий (numpy массив)
    Returns:
        Объединенное изображение с диаграммой
    """
    fig, ax = plt.subplots(figsize=(5, 2))
    actions = np.arange(len(probs))
    ax.bar(actions, probs, color='blue')
    ax.set_xlabel('Stations')
    ax.set_ylabel('Probability')
    ax.set_title('Station Probabilities (TRPO)')
    ax.set_ylim(0, 1)
    fig.tight_layout()
