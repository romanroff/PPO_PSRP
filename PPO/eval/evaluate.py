...
def evaluate_model(model, args):
    ...
    reward_data = {key: [] for key in ['capacity_rewards', 'distance_rewards', 'time_end_penalties', 
    ...
    while not done:
        ...
        for key in reward_data.keys():
            reward_data[key].append(info[0][key])
        ...
        if done:
            total_rewards = np.sum([np.array(reward_data[key]) for key in reward_data.keys()], axis=0)
            reward_data.update({'total_reward': total_rewards, 'new_day': new_day})
...
def evaluate_model_trpo(model, args):
    """
    Оценка модели TRPO с возвратом последовательности действий в формате [[idx_station, p1, p2, dayend],].
    Args:
        model: Обученная модель TRPO
        args: Аргументы конфигурации
    Returns:
        list: Последовательность действий в формате [[idx_station, p1, p2, dayend],]
    """
    # Инициализация среды и начальных параметров
    eval_env = model.get_env()
    obs = eval_env.reset()
    done = False
    num_envs = 1
    ...
    total_distance_fixed = False
    steps = 0
    action_sequence = []  # Список для хранения последовательности действий в формате [idx_station, p1, p2, dayend]
    capacities_list, new_day, image_arrays = [], [], []
    reward_data = {key: [] for key in ['capacity_rewards', 'distance_rewards', 'time_end_penalties', 
                                       'empty_load_penalties', 'restricted_station_penalties']}
    kpi_data = {key: [] for key in ['total_travel_distance', 'average_vehicle_utilization', 'average_stock_levels_percent', 
                                    'dry_runs', 'average_stops_per_trip',]}
    ...
    print('day_end', False)
    print('init_capacities:', eval_env.envs[0].init_capacities)
    print('vehicle_compartments after action:', eval_env.envs[0].temp_load)
    capacities_list.append(eval_env.envs[0].init_capacities[:, 0].tolist())
    ...
    while not done:
        ...
        # Извлечение idx_station
        idx_station = action.item() if np.isscalar(action) else action[0][1].item()

        # Извлечение day_end
        day_end = eval_env.envs[0].day_end if hasattr(eval_env.envs[0], 'day_end') else False

        # Сохранение действия в последовательность
        action_sequence.append(action.tolist()[0])

        # Шаг в среде
        obs, rewards, done, info = eval_env.step(action)
        # Создание пустого изображения для рендеринга
        width, height = [int(x * 1) for x in [100, 100]]  # Предполагаем, что 1 единица figsize = 100 пикселей
        img = np.zeros((height, width, 3), dtype=np.uint8)  # Черное изображение (RGB)
        frames = [Image.fromarray(img)]
        image_arrays.append(frames)  # Добавляем список кадров для текущего шага

        # Вывод информации о действии и состоянии среды
        if hasattr(eval_env.envs[0], 'day_end'):
            print('action', action)
            print('day end', eval_env.envs[0].day_end)
        print('init_capacities:', eval_env.envs[0].init_capacities)
        print('vehicle_compartments after action:', eval_env.envs[0].temp_load)
        print()

        # Добавление действия в множество посещенных
        visited_actions.add(idx_station)
        ...
        for key in kpi_data.keys():
            kpi_data[key].append(info[0].get(key, 0))  # Используем get с дефолтным значением 0, если ключ отсутствует

        # Сохранение данных о состоянии среды
        new_day.append(day_end)
        capacities_list.append(eval_env.envs[0].init_capacities[:, 0].tolist())
        ...
        if done:
            # Обработка reward_data
            total_rewards = np.sum([np.array(reward_data[key]) for key in reward_data.keys()], axis=0)
            reward_data.update({'total_reward': total_rewards, 'new_day': new_day})
            df_reward = pd.DataFrame(reward_data)
            log_kpi_metrics(df_reward, capacities_list, args, min_capacity=eval_env.envs[0].min_capacities[:, 0], 
                           max_capacity=eval_env.envs[0].max_capacities[:, 0])

            # Обработка kpi_data и сохранение в CSV
            df_kpi = pd.DataFrame(kpi_data)
            save_dir = f"results/trpo_{args.n}_{args.n_steps}_{args.veh}/steps"
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            df_kpi.to_csv(f'{save_dir}/kpi_data.csv', index=False)
