...
def log_kpi_metrics(df, capacities_list, args):
    fig, axes = plt.subplots(1, 1, figsize=(14*1.5, 12*1.5))
    # axes = axes.flatten()
    ...
    pd.DataFrame(capacities_list, columns=[f'Station {i+1}' for i in range(args.n - 1)]).plot(ax=axes, linewidth=2.0)
    axes.set_title("Station capacity", fontsize=title_fontsize)
    axes.grid()
    axes.set_ylabel("Liters", fontsize=label_fontsize)
    axes.set_xlabel("Steps", fontsize=label_fontsize)
    axes.tick_params(axis='x', labelsize=tick_fontsize)
    axes.tick_params(axis='y', labelsize=tick_fontsize)
    axes.set_ylim(0, 100)
    add_vertical_lines(axes, df['new_day'])
    axes.legend(loc="upper left", fontsize=legend_fontsize)  # Включаем легенду с номерами станций
